...
plt2 <- all_data %>% mutate(rentBurden_p1SE = rentBurden + 1.645*rentBurdenSE, 
                    rentBurden_m1SE = rentBurden - 1.645*rentBurdenSE) %>% 
  ggplot(aes(y=rentBurden, x=log(rentBurdenSE), color=high_rentBurden)) + 
  # geom_errorbar(width=0.1, alpha=0.6,  show.legend = F) + 
  geom_point( show.legend = F, alpha=0.8)+
  theme_bw(base_size = 11) + 
  theme(plot.title = element_text(size = rel(0.8)))+
    labs(title = TeX("(b): Log Variances vs.'True' Small Area Means"))  +
  xlab(TeX("$log(d_i)$"))+ ylab(TeX("Median Rent Burden, $z_i$"))
...
#ggsave("figs/figure0.png", dpi=600, dev='png', height=4.5, width=6.5, units="in") 
```
...
# ---- Load Chains ---- 
load("data_analysis/bym_res.RDA")
load("data_analysis/dm_res.RDA")
load("data_analysis/fh_res.RDA")
load("data_analysis/new_res.RDA")
...
tmp.df =  data.frame(area=1:nrow(all_data),
                     ...
                     SSD=exp(scale*new_res$theta+center) %>% 
                       apply(2, function(x){( (n-1) /n)*sd(x)}) %>% log()) 
